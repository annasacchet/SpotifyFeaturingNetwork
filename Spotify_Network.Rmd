...
cat("Assortatività per genere musicale:", round(assort_genre, 3), "\n")

# Assortatività per anno
assort_data <- analyze_by_year(g, 2009:2024, function(g_year) {
  genres <- V(g_year)$genres
  if (!is.null(genres) && length(unique(genres)) > 1) {
    assortativity_nominal(g_year, types = as.factor(genres))
  } else {
    NA
  }
})
...
ggplot(assort_data, aes(x = anno, y = valore)) +
  ...
  labs(title = "Evoluzione dell'assortatività per genere musicale",
